...
class Form20 extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            ...
            sectionForemanId : {valid : false, value : ""},
        }
    }
    ...
    sendForm = (event) => {
        ...
        fetch(`http://localhost:8080/form/19`,
            {
                method : "POST",
                mode : "cors",
                cache : "default",
                ...
                <form className={"row justify-content-center"}>
                    <div className={"row col-6 shadow-lg rounded-2"}>
                        <label htmlFor={`TypeCompanyField-${formNumber}`} className={"form-label"}>
                            <h4>Уникальные имя цеха и имя участка цеха, ключ начальника участка цеха</h4>
                        </label>
                        <div id={`TypeCompanyField-${formNumber}`}>
                            <div>
                                ...
                                <ValidTypeField formNumber={formNumber} fieldNumber={0} changeForm={this.handleFormChange} nameField={"workshopSectionName"} label={"Имя участка цеха:"} placeholder={"Введите уникальное имя участка цеха для редактирования, иначе выполнится вставка"} changeValidity={this.handleValidity} valid={this.state.workshopSectionName.valid}></ValidTypeField>
                                <ValidTypeField formNumber={formNumber} fieldNumber={0} changeForm={this.handleFormChange} nameField={"sectionForemanId"} label={"Уникальный ключ начальника участка цеха:"} placeholder={"Введите уникальный ключ начальника участка цеха"} changeValidity={this.handleValidity} valid={this.state.sectionForemanId.valid}></ValidTypeField>
                                <div className={"row"}>
                                    <div className={"col"}>
