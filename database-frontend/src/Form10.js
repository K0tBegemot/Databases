...
class Form10 extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            ...
            headerNames2 : ["Идентификационный номер оборудования", "Тип оборудования"],
            rowArray2 : [{testEquipmentId : "", testEquipmentTypeName : ""}],
    ...
    sendForm = (event) => {
        ...
        fetch(`http://localhost:8080/form/9`,
            {
                method : "POST",
                mode : "cors",
                cache : "default",
            ...
            this.setState({rowArray : response["statuses"]["statuses"], rowArray2 : response["equipments"]});
        });
    }

    addNewField = (event) => {
        event.preventDefault();
        let data = [...this.state.typeCompanyFields, {labNumber : "Все" , type : "Все", number : "Все", isValid : 1}];
        this.setState({typeCompanyFields : data});
    }
